...
export function ClientPagamentos() {
  ...
  async function togglePaid(p: PaymentItem) {
    ...
    try {
      ...
      const r = await fetch(`${BASE}/api/client/payments/${p.id}/${action}`, {
        method: "POST",
        credentials: "include",
      ...
      <div className="min-h-screen flex items-center justify-center px-4" style={{ background: "#07113A" }}>
        <div className="max-w-md w-full">
          <SessionExpiredBanner
            expired
            description="Sua sessão expirou. Faça login novamente para continuar acompanhando seus pagamentos."
